...
def TFs_targets(kbp, chip, MACS2, MAX):
    ...
    for url in chip.loc[:,TFrange]: #OBS! This takes a long time.
        while t < 60:
            try:
                ...
                print(f'Genes for {chip.index[dex]} ({dex}) found')
    ...
    TF_gene_sets.to_csv(f'../data/Transfactors/TF_gene_sets_top{MAX}_{MACS2}+MACS2_{kbp}kbp.tsv', sep='\t')
...
def csv_fix(csv):
    ...
    newfilename = f'{name}_CORRECTED{ext}'
...
def dataprep(expdata, remove=None, filtering=False):
    '''
...
def run_PCA(data, TF_gene_sets, n_pc=1, PCA_func=weighted_PCA):
    '''
    Function that uses selected PCA on supplied dataframe
    '''
    TFs = TF_gene_sets.index.intersection(data.index)
    Other_results = pd.DataFrame(index=TFs)
    PCA_per_TF = {}
    Contributions = {}
    standardizer = StandardScaler()
    for i in range (n_pc):
        PCA_per_TF[f'PC{i+1}'] = pd.DataFrame(index=data.columns)
    for TF in TFs:
        ...
        if len(genes)>2:
            ...
            for i in range(n_pc):
                PCA_per_TF[f'PC{i+1}'][TF] = res.iloc[:,i]
                PCA_per_TF[TF] = res.iloc[:,0]
                Other_results.loc[TF,'Gene count'] = len(genes)
                Other_results.loc[TF,f'Variance explained PC{i+1}'] = expl[i]*100
                PC_stand = standardizer.fit_transform(PCA_per_TF[f'PC{i+1}'].loc[:,[TF]])
                PC_stand_neg = standardizer.fit_transform(-PCA_per_TF[f'PC{i+1}'].loc[:,[TF]])
                Other_results.loc[TF,f'MSE PC{i+1}'] = min(mean_squared_error(TF_stand,PC_stand),mean_squared_error(TF_stand,PC_stand_neg))
        else:
            print(TF+' has less than 3 genes associated with it and is therefore disregarded.')
            Other_results.drop(TF)
    for i in range (n_pc):
        PCA_per_TF[f'PC{i+1}'].sort_index(inplace=True)
...
def save_exp(data, filename):
    ...
    '''
    if os.path.isdir(f'../exp/{date.today()}') == False:
        os.mkdir(f'../exp/{date.today()}')
    data.to_csv(f'../exp/{date.today()}/{filename}.tsv', sep='\t')
...
def main(TF_gene_sets, expdata, n_pc=1, PC=1, which='', remove=None, filtering=False):
    '''
    Main function for TF activity estimation, including validation method
    '''

    logdata = dataprep(expdata, remove=remove, filtering=filtering)

    if which in ('Random','Top',''):
        TFs = TF_selection(logdata,TF_gene_sets, which=which,n=10)
    else:
        TFs = which
    ...
    [PCA_results, Other_results, Contributions] = run_PCA(logdata,TF_gene_sets.loc[TFs,:],n_pc,PCA_func=PCA_func)
    return PCA_results, Other_results, Contributions, logdata

### Plotting/experiment functions ###
...
def save_fig(figure, name):
    ...
    if os.path.isdir(f'../exp/{date.today()}') == False:
        os.mkdir(f'../exp/{date.today()}')
    figure.savefig(f'../exp/{date.today()}/{name}.png', bbox_inches="tight")
...
def datadist(logdata):
    data = pd.DataFrame(data = stats.zscore(logdata.values,axis=1), columns=logdata.columns, index=logdata.index)
    bins = np.linspace(-3,3,20)
    fig, ax = plt.subplots(figsize=(10,10))
    plot = plt.hist(np.concatenate(data.loc[:,:].values), bins)
    ax.set_xlabel('z-score')
    ax.set_ylabel('Number of measured values')
    save_fig(fig,'logdatadist')
    return fig

def validation(PCA_results, Other_results, logdata, organ, PC, remove, filtering):
    for TF in Other_results.dropna().index:
        if organ == None:
            plot = activityplot(TF,PCA_results[f'PC{PC}'],logdata,hue='organ',style='organ')
        else:
            plot = activityplot(TF,PCA_results[f'PC{PC}'],logdata,organ=' '+organ)
        save_fig(plot,f'Activity_{TF}_{organ}_{remove}PCremoved_filtering_{filtering}')
        plot.show()

def testall(expdata, n_pc=1, PC=1, organ=None,remove=None,filtering=False):
    '''
    ...
    save_fig(fig,f'Fulldata_{organ}_{remove}PCremoved_filtering_{filtering}')
    return PCA_results, explained_var, contributions

def SVD(in_data, remove=None, filtering=False):
    standardizer = StandardScaler()
...
def Sample_behaviour_plot(data, u, s, vh, sep='time'):
    ...
    sns.set_style('white')
    plot = sns.lmplot(x='Eigengene 1', y='Eigengene 2', data=df, hue=sep, fit_reg=False, height=7)
    save_fig(plot, 'SampleBehaviour'+sep)
    return plot

def Sample_behaviour(expdata, organ=None, remove=None, filtering=False, sep='time'):
    data = dataprep(expdata, remove=remove, filtering=filtering)
    [u,s,vh] = SVD(data,remove=remove,filtering=filtering)
    plot = Sample_behaviour_plot(data,u,s,vh,sep)
    plot.show()
    save_fig(plot,f'Sample_behaviour_{organ}_{remove}_{filtering}_{sep}')

def genesethist(df, xlim=500):
    '''
    Funciton for creating a histogram showing the size distribution of TF gene sets
    '''
    print(f'Dataset contains {df.shape[0]} TFs')
...
def datacheck(TF_gene_sets, expdata, kbp, MACS2, MAX, n_pc=2,organ=None, filtering=False, remove=None, sets=False, varexp=[], genebehave=False):
    '''
    ...
    [PCA_results, Other_results, Contributions, logdata] = main(TF_gene_sets, expdata, n_pc=n_pc, remove=remove, filtering=filtering)
    ...
    for i in varexp:
        ...
        save_fig(plot, f'VarExpl_PC{i}_{kbp}_{MACS2}_{MAX}_{organ}_{remove}PCremoved_filtering_{filtering}')
...
def TFs_targets_porch(kbp, chip, MACS2, MAX):
    ...
    TF_gene_sets.to_csv(f'../data/Transfactors/TF_gene_sets_porch_{kbp}_{MACS2}.tsv', sep='\t')
    return TF_gene_sets
...
def fewgenesethist(df):
    ...
    plt.ylabel('Number of TFs')
    plt.xlim=[0,500]
    print(f'Dataset contains '+str(df.loc[df['Gene count'] <= 500].shape[0])+' TF(s) with 500 or fewer genes')
    return plt
